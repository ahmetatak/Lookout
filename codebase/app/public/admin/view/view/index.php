    ...
    if (isset($this->data["log"]) and !empty($this->data["log"])) {
      foreach ($this->data["log"] as $this->key => $this->value) {
        ...
        if ($this->value["log_action"] == "delete")
          echo   "<del><p class=" . $css . ">" . (isset($actionico) ? $actionico : '') . " | " . sprintf(_('%s (%s) %s  %s  (#%s) at  %s from  %s'), '<i class="fas fa-user"></i> ' . $this->value["username"], $this->value["fullname"], $action, $table, $this->value["log_data_id"], Time::get($this->value["log_datetime"], "d.m.Y H:i"), mb_substr($this->value["log_ip"], 0, 5)) . "</p></del>";

        else
          echo   "<p class=" . $css . ">" . (isset($actionico) ? $actionico : '') . " | " . sprintf(_('%s (%s) %s  %s  (#%s) at  %s from  %s'), '<i class="fas fa-user"></i> ' . $this->value["username"], $this->value["fullname"], $action, $table, $this->value["log_data_id"], Time::get($this->value["log_datetime"], "d.m.Y H:i"), mb_substr($this->value["log_ip"], 0, 5)) . "</p>";
      }
    } else
